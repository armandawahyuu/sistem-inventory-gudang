...
import { karyawanImportSchema } from "@/lib/validations/import-schemas";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            // Get existing employee NIKs for duplicate check
            const existingEmployees = await tx.employee.findMany({
                select: { nik: true },
            });
            const existingNiks = new Set(
                existingEmployees.map((e) => e.nik.toLowerCase())
            );
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                    ...
                    const validated = karyawanImportSchema.parse(row);

                    // Check if NIK already exists
                    if (existingNiks.has(validated.nik.toLowerCase())) {
                        skipped++;
                        continue;
                    }
                    ...
                    existingNiks.add(validated.nik.toLowerCase());
                    success++;
                } catch (error) {
                    const errors: string[] = [];
